...
from src.storage import load_articles, save_articles
...
def main() -> None:
    ...
    if Path("articles.json").exists():
        try:
            old = load_articles()
            previous_urls = {a.url for articles in old.values() for a in articles}
        except Exception:
            pass
    ...
    export_html(articles_by_company, summaries, previous_urls=previous_urls)
